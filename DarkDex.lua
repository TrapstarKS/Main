...
--[[
	New Dex
	Final Version
	Developed by LorekeeperZinnia
	Modified by Trapstar
...
local function string_ret(o, typ)
	...
	if not (typ == "table" or typ == "userdata") then return tostring(o) end
	mt = (getrawmetatable or getmetatable)(o)
	if not mt then return tostring(o) end
...
local function serialize_table(t, p, c, s)
	if typeof(t) ~= "table" then return format_value(t) end
	...
	local function localized_format(v, is_table) return is_table and (c[v][2] >= p) and serialize_table(v, p + 1, c, s) or format_value(v) end
		...
		local function main()
			...
			Explorer.ShowRightClick = function()
				...
				if env.setclipboard then
					...
					context:AddRegistered("COPY_NODE")
				end
				context:AddRegistered("INSERT_OBJECT")
				context:AddRegistered("SAVE_INST")
				context:AddRegistered("VIEW_FUNCTIONS")
				...
				if presentClasses["BasePart"] or presentClasses["Model"] then
					...
					context:AddRegistered("REQUEST_STREAM_AROUND")
				...
				if presentClasses["LuaSourceContainer"] then
					...
					context:AddRegistered("COPY_BASE64")
				end
			...
			Explorer.InitRightClick = function()
				...
				context:Register("REQUEST_STREAM_AROUND", {
					Name = "Request Stream Around",
						...
						for i = 1, #sList do
							...
							elseif isa(node.Obj, "Model") or isa(node.Obj, "Tool") then
								if node.Obj.PrimaryPart then
									cframe = node.Obj.PrimaryPart.CFrame + Settings.Explorer.TeleportToOffset
								else
									cframe = node.Obj:GetModelCFrame() + Settings.Explorer.TeleportToOffset
								end
							end
						...
						for i = 1, #sList do
							...
							elseif isa(node.Obj, "Model") or isa(node.Obj, "Tool") then
								if node.Obj.PrimaryPart then
									...
									plr:RequestStreamAroundAsync(node.Obj:GetModelCFrame().Position, 1)
									...
									hrp.CFrame = node.Obj:GetModelCFrame() + Settings.Explorer.TeleportToOffset
								end
								break
							end
						end
				...
				context:Register("SAVE_INST", {
					...
					OnClick = function()
						...
						synsaveinstance({
							...
							SaveBytecode = true,
							Object = sList[1].Obj,
							FilePath = sList[1].Obj.Name .. "_" .. os.time(),
						})
					end,
				})
				...
				context:Register("FIRE_TOUCHTRANSMITTER", {
					...
					OnClick = function()
						...
						for _, v in ipairs(selection.List) do
							if v.Obj and v.Obj:IsA("TouchTransmitter") then firetouchinterest(hrp, v.Obj.Parent, 0) end
						end
					end,
				})
				...
				context:Register("FIRE_CLICKDETECTOR", {
					...
					OnClick = function()
						...
						if not hrp then return end
						for _, v in ipairs(selection.List) do
							if v.Obj and v.Obj:IsA("ClickDetector") then fireclickdetector(v.Obj) end
						end
					end,
				})
				...
				context:Register("FIRE_PROXIMITYPROMPT", {
					...
					OnClick = function()
						...
						if not hrp then return end
						for _, v in ipairs(selection.List) do
							if v.Obj and v.Obj:IsA("ProximityPrompt") then fireproximityprompt(v.Obj) end
						end
					end,
				})
				...
				context:Register("SELECT_PLAYER", {
					Name = "Select Player",
					IconMap = Explorer.ClassIcons,
					Icon = 9,
					OnClick = function()
		...
		for obj in next,newNilRoots do
			...
			Explorer.InitViewFunctions = function(x, y)
				...
				if not context then
					...
					Explorer.ViewFunctionsContext = context
				end
				...
				local function onClick(fName)
					local m = ""
					for i = 1, #sList do
						local node = sList[i]
						local obj = node.Obj
						local class = obj.ClassName
						local path = clth(Explorer.GetInstancePath(obj))
				...
				local lastCategory = ""
				local alreadyAdded = {}
				for class, objList in next, Classes do
					local apiClass = API.Classes[class]
			...
			Explorer.InitViewConnections = function(x, y)
				...
				if not context then
					...
					Explorer.ViewConnectionsContext = context
				end
	...
	The main properties interface
		...
		local function main()
						...
						if not isMultiType then
							...
							for c = 1, #classList do
								...
								if not firstSet then
									...
									elseif IsTag then
										local tagVal = CollectionService:HasTag(obj, attributeName)
										if tagVal then
											firstObj = obj
											firstSet = true
										end
									...
									elseif IsTag then
										local _ = CollectionService:GetTags(obj)
										propVal = table.find(_, attributeName) and _[table.find(_, attributeName)]
										if propVal == nil then skip = true end
									else
			...
			Properties.ShowExplorerProps = function()
				...
				local ShowDeprecatedProperties, ShowHiddenProperties = Settings.Properties.ShowDeprecatedProperties, Settings.Properties.ShowHiddenProperties
				...
				local maxTags = Settings.Properties.MaxTags
				...
				for i = 1, #sList do
					...
					while apiClass do
						...
						if not foundClasses[APIClassName] then
							...
							for i = 1, #apiProps do
								...
								if (not tags.Deprecated or ShowDeprecatedProperties) and (not tags.Hidden or ShowHiddenProperties) then
					...
					if showingTags and tagCount < maxTags then
						local tags = CollectionService:GetTags(obj)
						for _, name in pairs(tags) do
							if not foundTags[name] then
								local tagProp = { IsTag = true, Name = "TAG_" .. name, AttributeName = name, DisplayName = _, Class = "Instance", ValueType = { Name = "string" }, Category = "Tags", Tags = {} }
			...
			Properties.NewPropEntry = function(index)
				...
				return {
					...
					GuiElems = {
						...
						-- ToggleTags = nameFrame.ToggleTags,
			...
			Properties.GetPropVal = function(prop, obj)
				...
				elseif prop.IsTag then
					local _ = CollectionService:GetTags(obj)
					propVal = table.find(_, prop.AttributeName) and _[table.find(_, prop.AttributeName)]
					if not propVal then return nil end

					local typ = typeof(propVal)
					local currentType = Properties.TypeNameConvert[typ] or typ
					if prop.ValueType.Name ~= currentType then return nil end
				else
			...
			Properties.Refresh = function()
				...
				for i = 1, maxEntries do
					...
					-- local toggleTags = guiElems.ToggleTags
					...
					if prop then
						...
						if prop.SpecialRow then
							...
							if prop.CategoryName then
								...
								-- local showingTags = Settings.Properties.ShowTags
								-- local showingTagsText = (showingTags and "[Setting: ON]" or "[Setting: OFF]")
								-- toggleTags.Position = UDim2.new(1, -85 - leftOffset, 0, entry.Position.Y.Offset)
								-- toggleTags.Text = showingTagsText
								-- toggleTags.TextColor3 = Settings.Theme.Text
								-- toggleTags.Visible = (prop.CategoryName == "Tags")
							else
								local propName = prop.Name
								local typeData = prop.ValueType
								local typeName = typeData.Name
								...
								if propObj then
									...
									elseif prop.IsTag then
										propCons[#propCons + 1] = CollectionService:GetInstanceAddedSignal(prop.AttributeName):Connect(function() Properties.DisplayProp(prop, i) end)
										propCons[#propCons + 1] = CollectionService:GetInstanceRemovedSignal(prop.AttributeName):Connect(function() Properties.DisplayProp(prop, i) end)
										propCons[#propCons + 1] = CollectionService.TagAdded:Connect(function() Properties.DisplayProp(prop, i) end)
										propCons[#propCons + 1] = CollectionService.TagRemoved:Connect(function() Properties.DisplayProp(prop, i) end)
									else
				...
				for i = 1, #sList do
					...
					if isa(obj, propClass) then
						pcall(function()
							...
							elseif prop.IsTag then
								local _ = CollectionService:GetTags(obj)
								root = table.find(_, attributeName) and _[table.find(_, attributeName)]
							else
								root = obj[propName]
							end
			...
			Properties.InitEntryStuff = function()
				Properties.EntryTemplate = create({
					...
					{ 13, "Frame", { BackgroundTransparency = 1, Name = "EnumArrow", Parent = { 8 }, Position = UDim2.new(1, -16, 0, 3), Size = UDim2.new(0, 16, 0, 16), Visible = false } },
					{ 14, "Frame", { BackgroundColor3 = Color3.new(0.86274510622025, 0.86274510622025, 0.86274510622025), BorderSizePixel = 0, Parent = { 13 }, Position = UDim2.new(0, 8, 0, 9), Size = UDim2.new(0, 1, 0, 1) } },
					{ 15, "Frame", { BackgroundColor3 = Color3.new(0.86274510622025, 0.86274510622025, 0.86274510622025), BorderSizePixel = 0, Parent = { 13 }, Position = UDim2.new(0, 7, 0, 8), Size = UDim2.new(0, 3, 0, 1) } },
					{ 16, "Frame", { BackgroundColor3 = Color3.new(0.86274510622025, 0.86274510622025, 0.86274510622025), BorderSizePixel = 0, Parent = { 13 }, Position = UDim2.new(0, 6, 0, 7), Size = UDim2.new(0, 5, 0, 1) } },
					{
						17,
						"TextButton",
						{
		...
		local function main()
			...
			ScriptViewer.Init = function()
				...
				save.MouseButton1Click:Connect(function()
					...
					if env.movefileas then env.movefileas(filename, ".txt") end
				end)
...
DefaultSettings = (function()
	...
	return {
		...
		Properties = {
			...
			ShowDeprecatedFunctions = false,
			ShowDeprecatedEvents = false,
...
Main = (function()
	...
	Main.SaveSettings = function()
		local Fields = {
			ShowDeprecatedProperties = Settings.Properties.ShowDeprecatedProperties,
			ShowHiddenProperties = Settings.Properties.ShowHiddenProperties,

			ShowDeprecatedFunctions = Settings.Properties.ShowDeprecatedFunctions,
			ShowDeprecatedEvents = Settings.Properties.ShowDeprecatedEvents,

			ShowAttributes = Settings.Properties.ShowAttributes,

			ShowTags = Settings.Properties.ShowTags,
		}
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			...
			rawXML = rawXML or game:HttpGet("https://raw.githubusercontent.com/MaximumADHD/Roblox-Client-Tracker/roblox/ReflectionMetadata.xml")
		else
	...
	Main.CreateApp = function(data)
		...
		app.Visible = true
		app.Parent = data.Container or Main.AppsContainer
	...
	Main.CreateMainGui = function()
		...
		Main.AppsSettingsGrid = Main.AppsSettings.UIGridLayout
		...
		local openSettings = gui.OpenButton.MainFrame.BottomFrame.Settings
		openSettings.MouseButton1Click:Connect(function() Main.OpenSettings(not Main.MainGuiSettingsOpen) end)
			...
			OnClick = function(callback)
				if callback then
					...
					cptsInputBegan = service.UserInputService.InputBegan:Connect(function(input, gpe)
						...
						local obj = guiObjects and guiObjects[1]
		...
		Main.CreateApp({
			Name = "Show Deprecated Properties",
			IconMap = Main.LargeIcons,
			Container = Main.AppsSettings,
			Icon = 9,
			Open = Settings.Properties.ShowDeprecatedProperties,
			OnClick = function()
		...
		Main.CreateApp({
			...
			Container = Main.AppsSettings,
			Icon = 9,
			Open = Settings.Properties.ShowHiddenProperties,
			OnClick = function()
		...
		Main.CreateApp({
			Name = "Show Deprecated Events",
			IconMap = Main.LargeIcons,
			Container = Main.AppsSettings,
			Icon = 9,
			Open = Settings.Properties.ShowDeprecatedEvents,
			OnClick = function()
		...
		Main.CreateApp({
			Name = "Show Deprecated Functions",
			IconMap = Main.LargeIcons,
			Container = Main.AppsSettings,
			Icon = 9,
			Open = Settings.Properties.ShowDeprecatedFunctions,
			OnClick = function()
		...
		Main.CreateApp({
			...
			Container = Main.AppsSettings,
			Icon = 9,
			Open = Settings.Properties.ShowAttributes,
			OnClick = function()
				Settings.Properties.ShowAttributes = not Settings.Properties.ShowAttributes
				Main.SaveSettings()
			end,
	...
	Main.SetupFilesystem = function()
		...
		if getcustomasset then
			if not isfile(assetPath) then pcall(function() writefile(assetPath, game:HttpGet("https://androssy.net/images/ClassImages.png")) end) end
	...
	Main.Init = function()
		...
		if Main.Elevated then
			...
			-- Maybe can use https://setup.rbxcdn.com/DeployHistory.txt
